// AutoCellCounter_Global_v123.ijm
...
for (k=0; k<Methods.length; k++){ //loop Methods
	for (i=0; i<list.length; i++) { //loop images
		...
		run("Convert to Mask");
		run("Watershed"); // added in v123
		run("Create Selection");
		close();
		selectWindow(Name+".tif");
		setThreshold(min, max);
		run("Restore Selection");
		run("Analyze Particles...", "size=0-Infinity pixel show=Nothing display clear include");
